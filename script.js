...
function loadGCOptions() {
  ...
  supportedGCs.forEach(gc => {
    ...
    if (gc === defaultGC) option.selected = true; // Set default GC as selected
...
function updateSelectedOptions() {
  ...
  const selectedVersion = document.getElementById('jdkVersion').value;
  const selectedGC = document.getElementById('gcType').value;
  // Add checked memory options
  if (memoryOptions) {
    memoryOptions.common.forEach((option, index) => {
      const checkbox = document.getElementById(`option-memory-common-${index}`);
      if (checkbox.checked) {
        if (option.editable) {
...
function clearOptions() {
  ...
  const gcOptionsContainer = document.getElementById('gcOptionsContainer');
  const memOptionsContainer = document.getElementById('memOptionsContainer');
  const gcOptionsHeader = document.getElementById('gcOptionsHeader');
...
loadMemoryOptions(); // Draw memory options dynamically